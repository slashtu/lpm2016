...
if ( ! class_exists( 'MPC_WC_Add_To_Cart' ) ) {
	class MPC_WC_Add_To_Cart {
		...
		function shortcode_template( $atts, $content = null ) {
			...
			$return = $tooltip != '' ? '<div class="mpc-tooltip-wrap ' . $classes_wrap .'" data-id="' . $css_id . '">' : '';
			$return .= '<div class="mpc-wc-add_to_cart-wrap' . $classes_wrap .'" data-id="' . $css_id . '">';
		...
		function shortcode_styles( $styles ) {
			...
			if ( count( $inner_styles ) > 0 ) {
				$style .= '[data-id="' . $css_id . '"] .mpc-wc-add_to_cart {';
			...
			if ( count( $inner_styles ) > 0 ) {
				...
				$style .= '[data-id="' . $css_id . '"] .mpc-atc__title .mpc-atc__icon {';
					$style .= join( '', $inner_styles );
				$style .= '}';
			}

			if ( $styles[ 'padding_css' ] ) {
				$style .= '[data-id="' . $css_id . '"] .mpc-atc__title,';
				$style .= '[data-id="' . $css_id . '"] .mpc-atc__title-hover {';
					$style .= $styles[ 'padding_css' ];
				$style .= '}';
			}

			if ( $temp_style = MPC_Snippets::css_background( $styles, 'hover' ) ) {
				$style .= '[data-id="' . $css_id . '"] .mpc-atc__background {';
					$style .= $temp_style;
				$style .= '}';
			}
			...
			if ( $styles[ 'hover_title_type' ] != 'none' && count( $inner_styles ) > 0 ) {
				$style .= '[data-id="' . $css_id . '"] .mpc-atc__title-hover .mpc-atc__icon {';
					$style .= join( '', $inner_styles );
				$style .= '}';
			}

			if ( $styles[ 'hover_border_css' ] ) {
				$style .= '[data-id="' . $css_id . '"]:hover .mpc-wc-add_to_cart {';
			...
			if ( $styles[ 'hover_background_offset' ] ) {
				...
				if ( $temp_style ) {
					$style .= '[data-id="' . $css_id . '"]:hover .mpc-atc__background {';
						$style .= $temp_style;
					$style .= '}';
				}
			}
			...
			if ( $this->atts[ 'loader_position' ] == 'inside' && $temp_style = MPC_Snippets::css_background( $styles, 'loader' ) ) { $inner_styles[] = $temp_style; }
			if ( count( $inner_styles ) > 0 ) {
				$style .= $this->atts[ 'loader_position' ] == 'inside' ? '[data-id="' . $css_id . '"] .mpc--loader {' : '[data-id="' . $css_id . '"] .mpc-wc-add_to_cart + .mpc--loader {';
					$style .= join( '', $inner_styles );
				$style .= '}';
			}
			...
			if ( count( $inner_styles ) > 0 ) {
				$style .= '[data-id="' . $css_id . '"] .mpc--success {';
					$style .= join( '', $inner_styles );
				$style .= '}';
			}
			...
			if ( $styles[ 'success_border' ] ) {
				$style .= '[data-id="' . $css_id . '"] [data-notice$="success"] {';
					$style .= 'border-color:' . $styles[ 'success_border' ] . ';';
				$style .= '}';
			}
			if ( $styles[ 'error_border' ] ) {
				$style .= '[data-id="' . $css_id . '"] [data-notice$="error"] {';
