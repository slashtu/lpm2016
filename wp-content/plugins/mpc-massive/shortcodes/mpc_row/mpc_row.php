...
if ( ! class_exists( 'MPC_Row' ) ) {
	class MPC_Row {
		...
		public  $g_atts   = array(); // ToDo: Find a better way for templatera issue
		private $atts     = array();
		...
		function prepare_atts( $atts, $content ) {
			if( stripos( $content, 'templatera' ) !== false ) {
				$this->g_atts = $atts;
			}
		}

		function verify_node( DOMNode $row ) { // ToDo: Find more themes related classes
			$valid_classes = apply_filters( 'ma/row/valid_classes', array( 'vc_row', 'wpb_row', 'x-container' ) ); // ToDo: add more theme based class
			$row_class = $row->getAttribute( 'class' );
			$is_valid  = false;

			foreach( $valid_classes as $valid_class ) {
				if( !$is_valid && strpos( $row_class, $valid_class ) !== false ) {
					return $row;
				}
			}
			...
			foreach( $nodes as $node ) {
				...
				foreach( $valid_classes as $valid_class ) {
					if( !$is_valid && strpos( $node_class, $valid_class ) !== false ) {
						return $node;
					}
				}
			}
		...
		public function row_output( $output, $shortcode, $atts ) {
			...
			if( !empty( $this->g_atts ) && stripos( $output, 'templatera' ) !== false ) { // ToDo: Find a better way for templatera issue
				$atts = $this->g_atts;
				$this->g_atts = '';
			}
			...
			$output_append .= MPC_Snippets::get_scripts();
			...
			// Append classes
			if( $row->getAttribute( 'class' ) . $this->classes ) {
				$row->setAttribute( 'class', $row->getAttribute( 'class' ) . $this->classes );
			}
			...
			// Link Block
			MPC_Snippets::create_link_block( $row, $this->atts[ 'url' ], $mpc_can_link );
		...
		function append_class( $output, $shortcode, $atts = '' ) {
			...
			// Append classes
			$row->setAttribute( 'class', $row->getAttribute( 'class' ) . ' mpc-row' );
		...
		function toggle_row() {
			if ( $this->atts[ 'toggle_enable' ] ) {
				$mpc_icon       = MPC_Snippets::parse_atts_icon( $this->atts, 'toggle' );
				$mpc_hover_icon = MPC_Snippets::parse_atts_icon( $this->atts, 'hover_toggle' );
		...
		function shortcode_map() {
			...
			$params = array_merge( $toggle_params, $block_params, $date_params, $first_overlay, $first_overlay_background, $first_overlay_background_scroll, $second_overlay, $second_overlay_background, $second_overlay_background_scroll, $parallax, $top_separator, $bottom_separator, $animation );
