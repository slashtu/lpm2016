...
vc_add_shortcode_param( 'mpc_layout_select', 'mpc_layout_select_settings', MPC_MASSIVE_URL . '/assets/js/mpc-params.js' );
function mpc_layout_select_settings( $settings, $value ) {
	...
	foreach( $layouts as $layout => $size ) {
		...
		$content = '<div class="mpc-layout-item" data-checked="' . $checked . '" data-size="' . esc_attr( $size ) . '" data-value="' . esc_attr( $layout ) . '">';
			$content .= '<img src="'. MPC_MASSIVE_URL . '/assets/images/sprites/' . esc_attr( $shortcode ) . '/' . esc_attr( $layout ) . '.png" alt="' . esc_attr( ucfirst( $layout ) ) . '" />';
