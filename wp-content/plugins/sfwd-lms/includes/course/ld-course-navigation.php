...
function learndash_previous_post_link( $prevlink='', $url = false ) {
	...
	if ( $post->post_type == 'sfwd-lessons' ) {
		$link_name = sprintf( __( 'Previous %s', 'learndash' ), LearnDash_Custom_Label::get_label( 'lesson' ) );
		$posts = learndash_get_lesson_list();
	} else if ( $post->post_type == 'sfwd-topic' ) {
		$link_name = sprintf( __( 'Previous %s', 'learndash' ), LearnDash_Custom_Label::get_label( 'topic' ) );
...
function learndash_next_post_link( $prevlink='', $url = false, $post = null ) {
	...
	if ( $post->post_type == 'sfwd-lessons' ) {
		$link_name = sprintf( __( 'Next %s', 'learndash' ), LearnDash_Custom_Label::get_label( 'lesson' ) );
		$course_id = learndash_get_course_id( $post );
		$posts = learndash_get_lesson_list( $course_id );
	} else if ( $post->post_type == 'sfwd-topic' ) {
		$link_name = sprintf( __( 'Next %s', 'learndash' ), LearnDash_Custom_Label::get_label( 'topic' ) );
		$lesson_id = learndash_get_setting( $post, 'lesson' );
		$posts = learndash_get_topic_list( $lesson_id );
	} else {
		return $prevlink;
