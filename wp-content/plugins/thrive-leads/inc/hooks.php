...
function tve_leads_enqueue_variation_scripts( $variation ) {
	...
	$form_type = explode( '|', $variation['tpl'] );
	$form_type = current( $form_type );

	/* include also the CSS for each form type design */
	$css_key = 'tve-leads-' . $form_type . '-' . str_replace( '.css', '', $config['css'] );
	$css_url = '';
	$css     = array(
...
//	$available['sendgridemail']   = 'Thrive_Dash_List_Connection_SendGridEmail';
