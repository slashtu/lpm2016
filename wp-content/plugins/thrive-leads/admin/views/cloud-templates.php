<?php if ( ! empty( $template_downloaded ) ) : ?>
	...
	<?php if ( ! empty( $is_update ) ) : ?>
		<?php echo __( 'Template updated', 'thrive-leads' ) ?>
	<?php endif ?>

	<input type="hidden" class="lp_code"
	       value="<?php echo (bool) $tpl['multi_step'] ? 'multi_step|' : '' ?><?php echo $form_type . '|' . $template_downloaded ?>"/>
...
<?php foreach ( $templates as $data ) : ?>
	...
	<div class="tve_tpl_item tve_tpl_cloud_item <?php echo $multi_class ?>">
		<span
			class="tve-tpl-<?php echo $data['form_type'] ?> tve_grid_cell tve_leads_template tve_click<?php echo $selected === $data['key'] ? ' tve_cell_selected' : '' ?>"
