...
class URE_Admin_Menu {
    ...
    public static function update_data() {
        ...
        if (!current_user_can('ure_admin_menu_access')) {
            $lib->notification = esc_html__('URE: Insufficient permissions to use this add-on','user-role-editor');
            return;
        }
        ...
        if ($ure_object_type!=='role' && $ure_object_type!=='user') {
            $lib->notification = esc_html__('URE: administrator menu access: Wrong object type. Data was not updated.', 'user-role-editor');
            return;
        }
        
        $ure_object_name = filter_input(INPUT_POST, 'ure_object_name', FILTER_SANITIZE_STRING);
        if (empty($ure_object_name)) {
            $lib->notification = esc_html__('URE: administrator menu access: Empty object name. Data was not updated', 'user-role-editor');
            return;
        }
        ...
        $lib->notification = esc_html__('Administrator menu access data was updated successfully', 'user-role-editor');
    }
    // end of update_data()
